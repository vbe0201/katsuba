use kobold_wad::{Archive, ArchiveError, Inflater};
...
fn subdir() -> Result<(), ArchiveError> {
    ...
    let data = inflater.decompress(archive.file_contents(file), file.uncompressed_size as _)?;
...
fn inflate_twice() -> Result<(), ArchiveError> {
    ...
    let a = inflater
        .decompress(archive.file_contents(file), file.uncompressed_size as _)?
        .to_owned();
    let b = inflater
        .decompress(archive.file_contents(file), file.uncompressed_size as _)?
        .to_owned();
